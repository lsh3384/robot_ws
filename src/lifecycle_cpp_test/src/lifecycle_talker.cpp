...
// lifecycleTalker는 LifecycleNode를 상속받는다.
// 몇가지 콜백함수들을 포함하고 있고, 이 콜백함수들은 현재 노드의 상태에 따라서 실행된다.
// 모든 lifecycle 노드들은 서비스들을 포함하고 있는데
// 이 서비스를 통해서 외부에서 노드의 상태 변화를 일으키고 컨트롤할 수 있다.
